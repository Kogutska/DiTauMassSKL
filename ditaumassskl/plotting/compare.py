...
from ..variables import HH_VARIABLES as VARIABLES
...
def draw_ratio(a, b, field, category,
    ...
    with plot.pad('main') as pad:
        ...
        leg = Legend([a, b], textsize=0.035)
        leg.Draw()
        # draw the category label
        if category is not None:
            label = ROOT.TLatex(
                pad.GetLeftMargin() + 0.04, 0.87,
                category.label + optional_label_text)
            label.SetNDC()
...
def draw_hists(
    ...
    label.SetTextSize(textsize)
    label.Draw("same")
    leg = Legend(hists, pad=c, textsize=0.035)
    # textsize=20, leftmargin=0.6, topmargin=0.6)
    leg.Draw('same')
